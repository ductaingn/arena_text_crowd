...
from .math_utils import get_norm
...
class DistanceField:
    ...
    def set_path(self, path, insert_n_points, filter_path_n_average, closed_path_flag):
        """Method to set the path to be followed
        path: sequence of points
        insert_n_points: number of points to be inserted
        filter_path_n_average: parameter for the verage filter
        closed_path_flag: flag to that indicates the path is closed
        """
        ...
        if filter_path_n_average > 0:
            self.path = self.filter_path(
                self.path, filter_path_n_average, closed_path_flag
            )
        ...
        for k in range(len(self.path)):
            d_temp = math.sqrt(
                (self.pos[0] - self.path[k][0]) ** 2
                + (self.pos[1] - self.path[k][1]) ** 2
                + (self.pos[2] - self.path[k][2]) ** 2
            )
            if d_temp < d:
                self.state_k = k
                d = d_temp

        # print("New path received by the controller (%d points)" %len(self.path))
    ...
    def set_equation(self, equation_str, u_i, u_f, closed_path_flag, N):
        ...
        for k in range(len(self.equation_samples)):
            d_temp = math.sqrt(
                (self.pos[0] - self.equation_samples[k][0]) ** 2
                + (self.pos[1] - self.equation_samples[k][1]) ** 2
                + (self.pos[2] - self.equation_samples[k][2]) ** 2
            )
            if d_temp < d:
                self.state_k = k
                d = d_temp
        ...
        # print("New path received by the controller (equation)")
    ...
    def field_from_equation(self, pos):
        ...
        u_star = self.golden_search(
            pos, self.state_u - self.state_u_delta, self.state_u + self.state_u_delta
        )
        self.state_u = u_star
        # print(self.state_u)
        ...
        if self.flag_follow_obstacle:
            closest_vec = [
                self.closest_world[0] - self.pos[0],
                self.closest_world[1] - self.pos[1],
                self.closest_world[2] - self.pos[2],
            ]

            Do = math.sqrt(
                closest_vec[0] ** 2 + closest_vec[1] ** 2 + closest_vec[2] ** 2
            )

            closest_hat = [
                closest_vec[0] / (Do + 1e-8),
                closest_vec[1] / (Do + 1e-8),
                closest_vec[2] / (Do + 1e-8),
            ]
            ...
            if (
                Do < self.switch_dist_0
                and (closest_vec[0] * Vx + closest_vec[1] * Vy + closest_vec[2] * Vz)
                > 0
            ):
                D_vec2 = [
                    -(closest_vec[0] - closest_hat[0] * self.epsilon),
                    -(closest_vec[1] - closest_hat[1] * self.epsilon),
                    -(closest_vec[2] - closest_hat[2] * self.epsilon),
                ]
                ...
                T2 = [
                    V[0] - alpha * V_dot_gad_D2 * grad_D2[0],
                    V[1] - alpha * V_dot_gad_D2 * grad_D2[1],
                    V[2] - alpha * V_dot_gad_D2 * grad_D2[2],
                ]
                ...
                if Do < self.switch_dist:
                    ...
                    # print "C"
                    theta = (Do - self.switch_dist) / (
                        self.switch_dist_0 - self.switch_dist
                    )
                    # print theta
    ...
    def field_from_points(self, pos):
        ...
        k_vec = [
            self.state_k - self.state_k_delta + i for i in range(self.state_k_delta)
        ]
        ...
        D_vec = [
            x - local_path[k_min][0],
            y - local_path[k_min][1],
            z - local_path[k_min][2],
        ]
        grad_D = [
            D_vec[0] / (D + 0.000001),
            D_vec[1] / (D + 0.000001),
        ...
        # numerically compute the tangent vector and normalize it
        T = [
            local_path[k2][0] - local_path[k1][0],
            local_path[k2][1] - local_path[k1][1],
            local_path[k2][2] - local_path[k1][2],
        ]
        ...
        if not self.closed_path_flag:
            if k_min == size_path - 1:
                # print("CHECK THIS: k_min:%s size_path-1:%s self.pos:%s local_path[k_min]:%s"
                #       %str(k_min) %str(size_path - 1) %str(self.pos) %str(local_path[k_min]))
                # Vx = 0
                # Vy = 0
                # Vz = 0
    ...
    def vec_field_path(self):
        """Compute the vector field at current position
        :return:
            Vx, Vy, Vz, reached_endpoint
        """
        ...
        if closed_path_flag:
            ...
            for i in range(path_size):
                ...
                for j in range(1, qty_to_insert + 1):
                    ...
                    px = (1 - alpha) * original_path[i][0] + alpha * original_path[iM][
                        0
                    ]
                    py = (1 - alpha) * original_path[i][1] + alpha * original_path[iM][
                        1
                    ]
                    pz = (1 - alpha) * original_path[i][2] + alpha * original_path[iM][
                        2
                    ]
    ...
    def filter_path(original_path, filter_path_n_average, closed_path_flag):
        ...
        if filter_path_n_average > size_original_path:
            print(
                "Parameter 'filter_path_n_average' seems to be too high! (%d)"
        ...
        if closed_path_flag:
            ...
            for i in range(size_original_path):
                for j in ids:
                    ...
                    px = new_path[i][0] + original_path[k][0] * float(
                        1.0 / filter_path_n_average
                    )
                    py = new_path[i][1] + original_path[k][1] * float(
                        1.0 / filter_path_n_average
                    )
                    pz = new_path[i][2] + original_path[k][2] * float(
                        1.0 / filter_path_n_average
                    )
                    new_path[i] = (px, py, pz)
        else:
